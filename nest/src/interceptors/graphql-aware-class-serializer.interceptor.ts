      ...
      if (operation === 'subscription') {
        ...
        if (isObservable(result)) {
          ...
          if (typeof asyncIterator !== 'function') {
            return observableToAsyncIterator(observable) as unknown as Observable<unknown>;
          }
        }
        return result;
      }
...
function observableToAsyncIterator<T>(observable: Observable<T>): AsyncIterableIterator<T> {
  ...
  const iterator: AsyncIterableIterator<T> = {
    next(): Promise<IteratorResult<T>> {
      if (queue.length > 0) {
        return Promise.resolve(queue.shift() ?? { value: undefined, done: true });
      }
