...
> `GOOGLE_VERTEX_API_KEY` 是旧变量名，但仍作为兼容 fallback 保留。`vertex-global:*` 的 project/global 模式也会读取它来发 `x-goog-api-key`，只是新配置应优先写 `AI_GOOGLE_VERTEX_API_KEY`。
...
### Vertex AI project/global 模式

Priority/Flex PayGo 的官方路径不是旧的 Express Mode URL。需要使用：
...
并把 model key 写成 `vertex-global:*`，例如：

```typescript
model: 'vertex-global:gemini-2.5-flash?tier=priority&vertexRequestType=shared'
```

完整语义见 [LLM Vertex AI 路由与 PayGo](./llm-vertex.md)。
