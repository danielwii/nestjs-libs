 ...
 * Provider 能力检查
 */
export function hasGenerateText(provider: ILLMProvider): provider is ILLMProvider & { generateText: NonNullable<ILLMProvider['generateText']> } {
  return typeof provider.generateText === 'function';
}

export function hasGenerateObject(provider: ILLMProvider): provider is ILLMProvider & { generateObject: NonNullable<ILLMProvider['generateObject']> } {
  return typeof provider.generateObject === 'function';
}

export function hasStreamText(provider: ILLMProvider): provider is ILLMProvider & { streamText: NonNullable<ILLMProvider['streamText']> } {
  return typeof provider.streamText === 'function';
}

export function hasStreamObject(provider: ILLMProvider): provider is ILLMProvider & { streamObject: NonNullable<ILLMProvider['streamObject']> } {
